...
```{r, include = FALSE}
# How many projects are there in the data set?
df %>% distinct(project) %>% nrow() # 4434
...
df %>% arrange(project, bid) %>% group_by(project) %>% 
  ...
  summarise(
    sdev_bid = sqrt(var(bid)), # NOTE: empirical or pop std dev?
    mean_bid = mean(bid), 
    no_bids = n(),
    win_bid = min(bid),
...
# Did any project dissapear?
df_agg %>% distinct(project) %>% nrow() # 4434
```
...
df_agg %>% select(project, CV, no_bids) %>% filter_all(any_vars(!is.infinite(df_agg$altrd))) %>% na.omit() -> df_agg_filtered
...
# Solve for number of actual cartels (basic algebra)

numerator <-  TP[[dc_ix]] * num_fair_pred - FN[[dc_ix]] * num_cartel_pred
denom <- TP[[dc_ix]] * TN[[dc_ix]] - FN[[dc_ix]] * FP[[dc_ix]]

num_fair_actual <- numerator/(denom)

num_cartel_actual = df_agg %>% nrow() - num_fair_actual
...
plot_dist <- function(data, contract, category_name){
  ...
  geom_histogram(alpha = 0.6, show.legend = FALSE,  binwidth = 365, position = "fill") +
    geom_rug(data = data %>% filter(pos == 1, contract_type == contract)) +
...
```{r}
df_agg %>% na.omit() %>% 
  mutate(year=floor(date/365)) %>% group_by(year, contract_type) %>% 
...
chow_test <- function(df, type, bp){
  ...
  sc <- sctest(df_sb$altrd ~ df_sb$anonymiseddate, type = "Chow", point = bp)
  return(sc)
}

bpoint <- which(df_sb$anonymiseddate == argmax_cv[2])
chow_test(df_sb, 2, bpoint)
...
X_lab <- df_agg %>% na.omit() %>%
  filter(probability_collusion > 0.8 | probability_collusion < 0.1) %>% 
  select(CV, altrd2, project)

y_lab <- df_agg %>% na.omit() %>% 
  filter(probability_collusion > 0.8 | probability_collusion < 0.1) %>% 
  select(probability_collusion, project) %>% 
  mutate(cartel = if_else(probability_collusion > 0.8, 0, 1),
         cartel = as.factor(cartel)) %>% select(cartel, project, probability_collusion)

X <- df_agg %>% filter(probability_collusion <= 0.8 & probability_collusion >= 0.1) %>% select(CV, altrd2, project, probability_collusion) %>% na.omit()

m1 <- EMLeastSquaresClassifier(as.data.frame(X_lab %>% select(-project)), y_lab$cartel, as.data.frame(X %>% select(-c(project, probability_collusion))))

X$preds2 <- m1@responsibilities

dat_lab <- left_join(X_lab, y_lab)
new_df <- bind_rows(dat_lab, X)

new_df %>% ggplot(aes(x = probability_collusion, y = preds2)) + geom_point()
```
```{r}
df %>% arrange(project, bid) %>% group_by(project) %>%
  summarise(
...
df_agg2 -> y
df_agg %>% select(project, sdev_bid, mean_bid, date, contract_type, CV) -> x

# TDOO: weird that select changes what gets plotted
df_agg %>% select(project, date, contract_type, CV) %>%
  na.omit() %>% 
  mutate(year=floor(date/365)) %>% group_by(year, contract_type) %>% 
  summarise(avg_cv = mean(CV)) %>%
  ggplot(aes(x = year, y = avg_cv, color = as.factor(contract_type))) + 
