...
First, check correlation between number of bids and the log of the winning bid.
```{r check_correlation}
...
We will be looking at boxplots, therefore we create a convenience function for future use:
```{r box_plot_function}
...
Most tenders are within the pure categories during years 1 to 10.
...
How does the coefficient of variation develop?
```{r}
