...
# Theoretical distribution of digits:
benford_freq <- function(digit){
  freq <- log(1 + 1/digit, base = 10)
  return(freq)
}

empirical_freq <- function(df, bid_var, num_digits){
  freq_vec <- df %>% rename(bid = bid_var) %>%
    mutate(first_digit = substr(paste(bid), 1, 2)) %>% 
    count(first_digit, name = "digit_count") %>%
    mutate(cases = nrow(df),
    emp_freq = digit_count/cases) %>% select(first_digit, emp_freq)
  return(freq_vec)
}

# Analyse years 1 to 6 for Strassenbau:
year_range <- 1:6
type <- 1
df_filt <- df %>% filter(anonymisedyear %in% year_range, 
                         contract_type == type)
df_filt_win <- df_agg %>% filter(year %in% year_range, 
                         contract_type == type)

freq_all <- empirical_freq(df_filt, "bid", 2) %>% rename(all = emp_freq)
freq_win <- empirical_freq(df_filt_win, "win_bid", 2) %>% rename(win = emp_freq)
freq_ben <- benford_freq(10:99)

freq_tab <- cbind(freq_ben, freq_all)
freq_tab <- left_join(freq_tab, freq_win, by=c("first_digit", "first_digit")) # not all values exist in the win vector

freq_tab %>%
  gather(key = freq, value = "value", c(win, freq_ben))%>% 
  ggplot(aes(x = as.integer(first_digit), y = value, color = freq)) + 
  geom_point() +
  theme_minimal() +
  labs(y = "", x = "First two significant digits", 
       title = "Frequency", 
       caption = "Plot shows winning bids between years 1 and 6 where contract type is Strassenbau") +
  scale_color_manual(name = "Distribution", values=c("skyblue", "coral"), labels = c("Theoretical", "Empirical"))
ggsave("images/benford.png")

chisq.test(x = freq_tab$win, y = freq_tab$freq_ben)
ks.test(x = freq_tab$win, y = freq_tab$freq_ben) # Significant at p = 0.00267 but this is true for seemingly fair periods too.
