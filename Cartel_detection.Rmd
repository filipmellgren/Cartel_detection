...
# Structural screen
```{r}
box_plot <- function(df, type, color, type_name){
  plot <- melt(df, id.var = c("contract_type", "year"), measure.vars = c("win_bid")) %>%
    filter(contract_type == type) %>% 
    ggplot(aes(x = as.factor(year), y = value)) + 
    geom_boxplot(outlier.shape = NA, fill = color) + theme_minimal() +
    labs(y = "", x = "Year", subtitle = type_name) +
    ylim(0, 10^6) 
  return(plot)
}

box1 <- box_plot(df_agg, 1, "#f8766d", "Strassenbau")
box2 <- box_plot(df_agg, 2, "#62B74E", "Strassen und Tiefbau")
box3 <- box_plot(df_agg, 3, "#00b0f6", "Tiefbau")
...
df_agg %>% 
  ...
  ggplot(aes(x=date2, y= no_bids, color = as.factor(contract_type),
             fill = as.factor(contract_type))) +
  geom_smooth() +
  theme_minimal() +
  labs(y = "Number of bids", x = "Year", 
  title = "Variation in number of bids over time", 
  caption = "- - - - indicate potential break point") +
  scale_color_discrete(name = "Contract type", labels = c("Strassenbau", "Strassen und Tiefbau", "Tiefbau"))
ggsave("images/number_bids.png", width = 24, height = 14, units = "cm")
