...
```{r regression_jump}
lm_sb <- function(df, type, bp){
  df <- df %>% filter(contract_type == type)
  df <- df %>% mutate(struc_break_str = if_else(date2 >= bp, 1, 0))
  lm_sb <- lm(altrd ~ date2*struc_break_str, data = df)
  fit_sb <- lm_sb$fitted.values
  df$fit <- fit_sb
  
  p <- df %>% ggplot(aes(x = date2)) + geom_point(aes(y = altrd)) +geom_line(aes(y = fit, color = "red"))
  plot(p)
  return(lm_sb)
}
# Strassenbau
sb1 <- lm_sb(df_agg, 1, 7)
summary(sb1)

# Strassen und Tiefbau
sb2 <- lm_sb(df_agg, 2, 6)
sb2
summary(sb2)
# Tiefbau
