...
df %>% arrange(project, bid) %>% group_by(project) %>% 
  ...
  mutate(
    CV = 100* (sdev_bid / mean_bid) * sqrt((no_bids-1)/(no_bids)), # Note that we multiply by 100
...
dist_cat1 / dist_cat2 / dist_cat3 + 
  labs(caption = "Note 1: Red indicates predicted status is cartel
       Note 2: ---- denotes the fraction of expected predicted cartels given no cartel (false positives)", x = "Year")
