...
df %>% arrange(project, bid) %>% group_by(project) %>% 
  ...
  summarise(
    mean_bid = mean(bid), 
    no_bids = n(),
    sdev_bid = sqrt(var(bid)*(no_bids-1)/(no_bids)), # NOTE: var() defaults t0 denom n-1, want n.
    win_bid = min(bid),
...
# Plot CV over time
```{r}
str_cv <- df_agg %>% filter(contract_type == 1) %>% 
  ggplot(aes(x = date, y = CV)) +
  geom_point()

mix_cv <- df_agg %>% filter(contract_type == 2) %>% 
  ggplot(aes(x = date, y = CV)) +
  geom_point()

tief_cv <- df_agg %>% filter(contract_type == 3) %>% 
  ggplot(aes(x = date, y = CV)) +
  geom_point()
...
prob_col_time <- df_agg %>%
  ggplot(aes(x = date, y = probability_collusion, color = as.factor(contract_type)), alpha = 1) +
geom_vline(aes(xintercept = argmin_cv[1]), 
             linetype="dotted", size=1.5, color = "red", alpha = 0.5) +
  geom_smooth(aes(fill = as.factor(contract_type)), alpha = 0.4) +
  theme_minimal() + 
